...
module.exports = {
    ...
    async execute(interaction) {
        ...
        async function redditMeme() {
            await fetch('https://www.reddit.com/r/memes/random/.json').then(async res => {
                ...
                return interaction.reply({
                    embeds: [embed.setTitle(title).setImage(url).setURL(url).setColor("Random").setFooter({
                        text: author
                    })]
                });
            });
        ...
        async function giphyMeme() {
            await fetch('https://api.giphy.com/v1/gifs/random?api_key=t8rkLOfXSdKGS1PBcWf5gq4vBnXhyeQY&tag=&rating=g').then(async res => {
                let meme = await res.json();
